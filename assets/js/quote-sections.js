(function ($) {
    ...
    function appendDeletedKey(key) {
        var $deleted = $('#tah-quote-sections-deleted');
        if (!$deleted.length || !key) {
            return;
        }

        var current = String($deleted.val() || '');
        var items = current ? current.split(',') : [];
        if (items.indexOf(key) === -1) {
            items.push(key);
        }
        $deleted.val(items.join(','));
    }
    ...
    function buildQuoteSectionItem(key, title, labels, isLocal) {
        ...
        var toggleLabel = (labels && labels.hideSection) || 'Hide section';
    ...
    function setEnabledButtonState($item, enabled, labels) {
        var $input = $item.find('.tah-section-enabled-input');
        var $button = $item.find('.tah-toggle-enabled');
        var $icon = $button.find('.dashicons');
        var isEnabled = !!enabled;
        var label = isEnabled ? ((labels && labels.hideSection) || 'Hide section') : ((labels && labels.showSection) || 'Show section');

        $input.val(isEnabled ? '1' : '0');
        $icon.attr('class', isEnabled ? 'dashicons dashicons-visibility' : 'dashicons dashicons-hidden');
        $button.attr('aria-label', label).attr('title', label);
        $item.toggleClass('tah-section-disabled', !isEnabled);
    }

    function initializeEnabledStates($scope, labels) {
        $scope.find('.tah-quote-section-item').each(function () {
            var $item = $(this);
            if ($item.hasClass('tah-create-section-item')) {
                return;
            }
            var current = String($item.find('.tah-section-enabled-input').val() || '1');
            var isEnabled = !['0', 'false', 'off'].includes(current);
            setEnabledButtonState($item, isEnabled, labels);
        });
    }
    ...
    function addLocalSectionFromTitle($list, title, labels) {
        ...
        setEnabledButtonState($item, true, labels);
        syncOrderInput();
        refreshEmptyState();
        $item.find('.tah-local-title-input').trigger('focus');
    }

    function moveActionsToHandle() {
        var $dropdown = $('.tah-actions-dropdown');
        var $postbox = $dropdown.closest('.postbox');
        var $handleActions = $postbox.find('.handle-actions');

        if ($dropdown.length && $handleActions.length) {
            $handleActions.prepend($dropdown);
        }
    }
    ...
    $(function () {
        ...
        if (typeof tahQuoteSectionsConfig !== 'undefined' && $list.length) {
            $(document).on('change', 'input[name="tah_trade_term_id"]', function () {
                ...
                initializeEnabledStates($list, labels);
                syncOrderInput();
            ...
            initializeEnabledStates($list, tahQuoteSectionsConfig.labels || {});
            refreshToolsAndHeader();
            refreshEmptyState();
            refreshEmptyState();
            toggleCreateControls($list);
        ...
        $(document).on('click', '.tah-actions-toggle', function (event) {
            event.preventDefault();
