		...
		<main id="archive_content" class="archive_content">
			...
			if ($query->have_posts()) {
				...
				if (function_exists("pagination")) {
					pagination(isset($additional_loop) && $additional_loop->max_num_pages);
				}
			} else {
				?>
				<article id="post-0" class="post no-results not-found">
					<?php if (current_user_can('edit_posts')):  // Show a different message to a logged-in user who can add posts. 
						...
						<div class="entry-content">
							<p><?php printf(__('Ready to publish your first post? <a href="%s">Get started here</a>.', 'the-artist'), admin_url('post-new.php')); ?>
							</p>
						</div><!-- .entry-content -->
					...
					<?php else:  // Show the default message to everyone else. 
						...
						<div class="entry-content">
							<p><?php _e('Apologies, but no results were found. Perhaps searching will help find a related post.', 'the-artist'); ?>
							</p>
							<?php get_search_form(); ?>
						</div><!-- .entry-content -->
