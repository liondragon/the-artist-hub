...
function paginate($ajax = false)
{
	...
	if (1 === $max_pages) {
		...
		if ($ajax) {
			...
			$next_text = 'Load More';
			$spinner = '<div class="spinner" style="display:none;"><svg class="loader-icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="84px" height="84px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><path d="M10 50A40 40 0 0 0 90 50A40 41.1 0 0 1 10 50" stroke="none"><animateTransform attributeName="transform" type="rotate" dur="1s" repeatCount="indefinite" keyTimes="0;1" values="0 50 50.55;360 50 50.55"></animateTransform></path></svg></div>';
		} else {
		...
		$prev = ($current_page !== 1) ? '<a class="prev arrow" href="' . get_pagenum_link($current_page - 1) . '" aria-label="Previous Page">« Previous</a>' : '<a href="#" class="prev disabled" aria-disabled="true">« Previous</a>';
		$next = ($current_page !== $max_pages) ? '<a class="' . $next_class . '" href="' . get_pagenum_link($current_page + 1) . '" data-pages="' . $max_pages . '" aria-label="Next Page">' . $next_text . '</a>' : '<a href="#" class="' . $next_class . ' disabled" aria-disabled="true">' . $next_text . '</a>';
		$fdots = ($current_page > 5 && $max) ? '<span class="dots" aria-hidden="true">…</span>' : (($current_page == 5 && $max) ? '<a href="' . get_pagenum_link(2) . '">2</a>' : '');
		$ldots = ($current_page < $max_pages - 4 && $max) ? '<span class="dots" aria-hidden="true">…</span>' : (($current_page == $max_pages - 4 && $max) ? '<a href="' . get_pagenum_link($max_pages - 1) . '">' . $plast . '</a>' : '');
		$links = implode('', $pages);
		$pagination = '<nav class="pagination" aria-label="Pagination">' . ($ajax ? '' : $prev . $first . $fdots . $links . $ldots . $last) . ($ajax && $current_page == $max_pages ? '' : $next) . '</nav>' . $spinner;
	}
	echo $pagination;
}
