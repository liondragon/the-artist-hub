...
//Metabox
add_action('admin_menu', 'the_artist_vehicle_add_metabox');
// or add_action( 'add_meta_boxes', 'the_artist_vehicle_add_metabox' );
// or add_action( 'add_meta_boxes_{post_type}', 'the_artist_vehicle_add_metabox' );

function the_artist_vehicle_add_metabox()
{

	add_meta_box(
		'tfa_metabox', // metabox ID
		'Customer Information', // title
		'the_artist_vehicle_metabox_callback', // callback function
...
function the_artist_vehicle_metabox_callback($post)
{
	...
	wp_nonce_field('the_artist_save_vehicle', '_tfanonce');
...
add_action('save_post', 'the_artist_vehicle_save_meta', 10, 2);
// or add_action( 'save_post_{post_type}', 'the_artist_vehicle_save_meta', 10, 2 );

function the_artist_vehicle_save_meta($post_id, $post)
{

	// nonce check
	if (!isset($_POST['_tfanonce']) || !wp_verify_nonce($_POST['_tfanonce'], 'the_artist_save_vehicle')) {
		return $post_id;
	}
