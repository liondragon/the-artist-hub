...
//Add a Metabox
add_action( 'admin_menu', 'tfa_add_project_metabox' );
function tfa_add_project_metabox() {
	add_meta_box(
		'tfa_project_metabox', // metabox ID
		'Project Information', // title
		'tfa_project_metabox_callback', // callback function
...
function tfa_project_metabox_callback( $post ) {
	...
	wp_nonce_field( 'somerandomstr', '_tfanonce_project' );
...
add_action( 'save_post', 'tfa_project_save_meta', 10, 2 );
// or add_action( 'save_post_{post_type}', 'tfa_project_save_meta', 10, 2 );

function tfa_project_save_meta( $post_id, $post ) {

	// nonce check
	if ( ! isset( $_POST[ '_tfanonce_project' ] ) || ! wp_verify_nonce( $_POST[ '_tfanonce_project' ], 'somerandomstr' ) ) {
		return $post_id;
	}
...
//ADD CUSTOM COLUMNS
	...
	add_filter('manage_projects_posts_columns', function($columns) {
    	$columns['title'] = __('Project Title', 'textdomain');
    	return $columns;
    });

    add_filter('manage_projects_posts_columns', function($columns) {
    	return array_merge($columns,  ['project_start_date' => __('Start Date', 'textdomain')], ['project_customer_name' => __('Client Name', 'textdomain')], ['project_address' => __('Address', 'textdomain')], ['subcontractor' => __('Team', 'textdomain')], ['project_total_amount' => __('Labor', 'textdomain')]);
    });
    ...
    add_action('manage_projects_posts_custom_column', function($column_key, $post_id) {
		if ($column_key == 'project_start_date') {
    		...
    		if ($project_start_date) {
    			echo '<span>'; _e($project_start_date, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
		...
		if ($column_key == 'project_customer_name') {
    		...
    		if ($project_customer_name) {
    			echo '<span>'; _e($project_customer_name, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
    	...
    	if ($column_key == 'project_address') {
    		...
    		if ($project_address) {
    			echo '<span>'; _e($project_address, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
	   	...
	   	if ($column_key == 'subcontractor') {
    		...
    		if ($subcontractor) {
    			echo '<span>'; _e($subcontractor, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
	   	...
	   	if ($column_key == 'project_total_amount') {
    		...
    		if ($project_total_amount) {
    			echo '<span>'; _e($project_total_amount, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
