...
//Add a Metabox
add_action( 'admin_menu', 'tfa_add_equipment_metabox' );
function tfa_add_equipment_metabox() {
	add_meta_box(
		'tfa_equipment_metabox', // metabox ID
		'Equipment Information', // title
		'tfa_equipment_metabox_callback', // callback function
...
function tfa_equipment_metabox_callback( $post ) {
	...
	wp_nonce_field( 'somerandomstr', '_tfanonce_equipment' );
...
add_action( 'save_post', 'tfa_equipment_save_meta', 10, 2 );
// or add_action( 'save_post_{post_type}', 'tfa_equipment_save_meta', 10, 2 );

function tfa_equipment_save_meta( $post_id, $post ) {

	// nonce check
	if ( ! isset( $_POST[ '_tfanonce_equipment' ] ) || ! wp_verify_nonce( $_POST[ '_tfanonce_equipment' ], 'somerandomstr' ) ) {
		return $post_id;
	}
...
//ADD CUSTOM COLUMNS
	...
	add_filter('manage_equipment_posts_columns', function($columns) {
    	$columns['title'] = __('Equipment Title', 'textdomain');
    	return $columns;
    });

    add_filter('manage_equipment_posts_columns', function($columns) {
    	return array_merge($columns,  ['equipment_purchase_date' => __('Purchase Date', 'textdomain')], ['equipment_name' => __('Name', 'textdomain')], ['model_n' => __('Model', 'textdomain')], ['eq_price' => __('Purchase Price', 'textdomain')], ['eq_own' => __('Ownership', 'textdomain')], ['equipment_warranty' => __('Warranty', 'textdomain')]);
    });
    ...
    add_action('manage_equipment_posts_custom_column', function($column_key, $post_id) {
		if ($column_key == 'equipment_purchase_date') {
    		...
    		if ($equipment_purchase_date) {
    			echo '<span>'; _e($equipment_purchase_date, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
		...
		if ($column_key == 'equipment_name') {
    		...
    		if ($equipment_name) {
    			echo '<span>'; _e($equipment_name, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
    	...
    	if ($column_key == 'model_n') {
    		...
    		if ($model_n) {
    			echo '<span>'; _e($model_n, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
	   	...
	   	if ($column_key == 'eq_price') {
    		...
    		if ($eq_price) {
    			echo '<span>'; _e($eq_price, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
	   	...
	   	if ($column_key == 'eq_own') {
    		...
    		if ($eq_own) {
    			echo '<span>'; _e($eq_own, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
	   	...
	   	if ($column_key == 'equipment_warranty') {
    		...
    		if ($equipment_warranty) {
    			echo '<span>'; _e($equipment_warranty, 'textdomain'); echo '</span>';
    		} else {
    			echo 'Not Provided';
    		}
    	}
